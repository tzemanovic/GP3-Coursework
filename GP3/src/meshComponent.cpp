...
void MeshComponent::Mesh::init( const std::vector<Vertex>& vertices, const std::vector<unsigned>& indices )
{
...
MeshComponent::MeshComponent( String&& filename ) : RenderComponent( ComponentType::Mesh ), _filename( filename ), _shaders( nullptr )
{
...
void MeshComponent::init( const aiScene* scene )
{
	...
	{
		initMesh( _meshes[i], scene->mMeshes[i] );
	}
	// initialise textures
	for ( unsigned i = 0; i < scene->mNumMaterials; ++i )
	{
		initTexture( scene->mMaterials[i] );
	}
}
void MeshComponent::initMesh( Mesh& mesh, const aiMesh* importMesh )
{
...
void MeshComponent::initTexture( const aiMaterial* importTexture )
{
	aiString path;
